 ...
 */
export function createOffscreenCache(): OffscreenCache {
  return {
    canvas: null,
    ctx: null,
    needsUpdate: true
 ...
 */
export function initOffscreenCanvas(
 ...
 */
export function drawCachedElements(
 ...
 */
export function invalidateCache(cache: OffscreenCache): void {
  cache.needsUpdate = true;
};
 ...
 * Очищает кэш
 */
export function destroyCache(cache: OffscreenCache): void {
  if (cache.canvas) {
    cache.canvas = null;
    cache.ctx = null;
  }
 ...
 */
export function isCacheReady(cache: OffscreenCache): boolean {
